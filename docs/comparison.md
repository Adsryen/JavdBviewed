...
本轮范围：重新比对 `reference/Tampermonkey` 中更新的三个参考脚本，并同步当前拓展已完成的字幕搜索、详情页 Portal 和迅雷字幕弹窗状态。
...
当前拓展已经覆盖参考脚本中对 JavDB 详情页价值最高的能力：超级排行榜、热播、Top250、FC2 弹窗、评论解锁、相关清单解锁、JAVBUS/多源磁力、在线可看性检测、详情页外部搜索、字幕搜索、迅雷字幕弹窗、115 推送、WebDAV 备份恢复、演员备注和新作品检测。
...
| 多源磁力聚合 | [已落地] | JavdbBuddy、JAV-JHS、FC2PPVDB-Enhanced | 支持 JavDB、Sukebei、BTdig、BTSOW、JAVBUS，统一去重、来源 tag、来源筛选、分页、toast 数量提示 | FC2PPVDB-Enhanced 有负缓存、队列超时、批量提取；JAV-JHS 有独立 MagnetHub 弹窗 | 建议补负缓存和失败退避；批量提取暂缓 |
...
| Emby/Jellyfin 入库状态 | [部分落地] | JavdbBuddy | 当前有 Emby/Jellyfin 页面增强和 WebDAV/数据管理 | 参考脚本会同步媒体库索引，并在 JavDB 列表/详情页显示 Emby/Jellyfin 入库状态与跳转链接 | 建议作为独立大功能设计 |
| 实时媒体库校验 | [待做] | JavdbBuddy | 当前缺少 JavDB 列表项逐个查询媒体服务器的队列 | 参考脚本可实时修正本地索引 | 建议跟 Emby/Jellyfin 入库状态一起设计 |
| 鉴定记录/收藏/屏蔽/已下载/已观看 | [部分落地] | JAV-JHS、FC2PPVDB-Enhanced | 当前拓展有已浏览/想看/已看、本地记录、番号库和内容过滤 | JAV-JHS 的四状态表格、批量变更、快捷键、列表卡片动作更完整；FC2PPVDB-Enhanced 有 watched/downloaded 跨站状态 | 建议只吸收“批量状态变更”和“状态筛选 UX”，避免重复实现整套 JHS |
...
| 列表页卡片工具栏 | [部分落地] | JAV-JHS、FC2PPVDB-Enhanced | 当前有列表增强、预览、状态标记、内容过滤 | 参考脚本有长缩略图、视频播放、复制番号/标题/封面、第三方站、状态按钮、批量选择 | 建议优先补“复制/下载封面/第三方站”轻量按钮 |
...
| 批量选择/批量磁力提取 | [待做] | FC2PPVDB-Enhanced | 当前番号库有批量操作，页面卡片侧批量选择能力有限 | 参考脚本卡片上可批量标记已看、取消已看、复制 ID、提取磁力 | 建议在“列表页卡片工具栏”后评估 |
...
- 当前拓展已覆盖其主链路，剩余高价值差异集中在 Emby/Jellyfin 入库状态。
...
| 优先级 | 功能 | 建议动作 | 原因 |
|---:|---|---|---|
| 1 | Emby/Jellyfin 入库状态 | 单独做规格设计 | 参考脚本剩余差异里价值最高，涉及数据模型、索引同步、JavDB 页面标记、实时校验 |
