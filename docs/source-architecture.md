...
WebDAV 恢复页拆分按“业务规则进入 feature、Dashboard 展示模型留在 dashboard 子目录”推进：旧备份格式识别与迁移进入 `features/webdavSync/application/backupMigration.ts`，恢复文件列表的大小、相对时间、日期范围和上传者展示模型进入 `dashboard/webdavRestore/fileListModel.ts`，`dashboard/webdavRestore.ts` 保留弹窗流程和 DOM 装配。
