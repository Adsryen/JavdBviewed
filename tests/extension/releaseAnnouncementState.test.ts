...
describe('release announcement state', () => {
  ...
  it('ships user-facing release notes for recent versions', () => {
    expect(RELEASE_NOTES.map(note => note.version)).toEqual(['1.21.2', '1.21.1', '1.21.0', '1.20.2', '1.20.1', '1.20.0']);
    ...
    expect(RELEASE_NOTES[4]?.highlights).toContain('115 离线下载支持选择目标文件夹。');
    expect(RELEASE_NOTES[5]?.highlights).toContain('全局任务中心接入详情页增强和后台任务。');
