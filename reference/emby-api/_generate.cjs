 ...
 *
 * Usage: node reference/emby-api/_generate.cjs
 */
const fs = require('fs');
...
for (const tag of sortedTags) {
  ...
  md += `# ${cn}（${tag}）\n\n`;
  md += `> 分类：${cat}\n`;
  md += `> 来源：用户 Emby Server 4.9.5.0 的官方 OpenAPI（\`GET {server}/openapi.json\`）\n`;
  md += `> 抓取基址示例：http://47.108.74.231:38096/openapi.json\n`;
  md += `> 规范版本：${esc(j.info?.version)} · 接口数：${ops.length}\n\n`;
  md += `## 接口列表\n\n`;
...
- 本目录 \`openapi.json\` 抓取自 \`swagger.emby.media\`，info.version = **${esc(j.info?.version)}**
- 若需与本机完全一致，请用你服务器的 \`/openapi.json\` 覆盖并重新运行 \`node _generate.cjs\`
`;
fs.writeFileSync(path.join(root, '认证.md'), authMd, 'utf8');

const quick = `# 常用接口速查（本项目相关）
...
| --- | --- | --- |
| Recursive | true | 递归 |
| IncludeItemTypes | Movie | 仅电影 |
| Fields | Path,PrimaryImageAspectRatio,ImageTags | 额外字段 |
| SearchTerm | 可选 | 关键字搜索 |
| api_key | *** | ApiKey |
...
见 \`接入认证/用户管理-UserService.md\` → \`postUsersAuthenticatebyname\`。

## POST /Library/Refresh

触发媒体库扫描（管理员）。

见 \`媒体库与条目/媒体库浏览-LibraryService.md\` → \`postLibraryRefresh\`。
`;
...
let readme = `# Emby Server API 文档（本地转存）
...
1. **OpenAPI 3 规范（原始文件）**：[\`openapi.json\`](./openapi.json)
   - 抓取自用户服务端：\`GET http://47.108.74.231:38096/openapi.json\`
   - 备份副本：[\`openapi-from-server-4.9.5.0.json\`](./openapi-from-server-4.9.5.0.json)
   - 标题：${esc(j.info?.title)}
   - 版本：${esc(j.info?.version)}（已与 \`/System/Info/Public\` 一致）
2. **服务器也可直接提供规范**：\`GET {server}/openapi.json\` / \`GET {server}/swagger.json\`
3. **开发者门户（概念文档）**：https://dev.emby.media/
4. **公共 Swagger 镜像已过时**（仍为 4.1.1.0，勿再当权威）：https://swagger.emby.media/openapi.json

> 本目录当前快照对应 **Emby Server 4.9.5.0**。以后升级 Server 后，请重新拉取 openapi 并运行生成脚本。
...
\`\`\`powershell
# 可选：用本机 Emby 最新规范覆盖
curl.exe -L -o reference/emby-api/openapi.json "http://YOUR_EMBY:8096/openapi.json"

# 重新生成全部分类 Markdown
node reference/emby-api/_generate.cjs
\`\`\`
`;
