...
- [ ] WebDAV 备份需要做好预警功能，超过多久没有备份，需要预警提示（待决定：弹窗还是 dashboard 的全局弹幕，需要评估）（**待验证**）
- [ ] 自动上传的功能失效了（**待验证**）
...
- [ ] 增加点击"想看"按钮，更新番号库影片状态，如果番号库没有该影片，则添加（**待验证**）
  ...
  - [ ] 点击"看过"按钮，状态更新为"已观看"
- 清单
  - [ ] 同步javdb的清单列表，数据同步页面需要有同步按钮
  - [ ] 为番号库中的影片新增 清单 归属，每个影片可以归属于多个清单
  - [ ] 在番号库页面，增加清单筛选功能
  - [ ] 番号库新增一个子页面，用于管理清单列表，清单可点击查看下面的影片，与番号库的清单搜索共存；番号库的影片列表新增显示方式（类似tag那种感觉？）
---
