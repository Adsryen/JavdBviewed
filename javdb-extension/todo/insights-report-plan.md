...
- [ ] AI：使用一套 HTML 模板，由 AI 填充“文本区块”；图表均本地渲染（不交由 AI 生成）。（模板/本地图表已完成；AI 接入未完成）
- [x] 触发：提供“每月报告”自动生成与持久化；支持查看历史与备份/恢复。
- [ ] 展示：在站点首页注入入口按钮，点击后打开扩展弹窗（Popup）进行生成/查看/导出等操作。（首页入口未实现；Popup 报告页已实现）
...
2) 聚合
- [ ] 将日记录按月聚合，产出：标签 TopN、占比、日/周趋势、显著变化（新偏好/上升/下降）。（TopN/趋势已完成；显著变化未实现）

3) 生成
- [x] 本地统计计算 + 本地 ECharts 渲染图表（默认使用 ECharts）。
- [ ] 将结构化统计数据 + HTML 模板（src/assets/templates/insights-report.html）传递给 AI，仅填充“文本占位符”（标题/摘要/洞察/方法说明等）。
...
- AI 输出：
  - [ ] 首选：返回“整段 HTML（仅替换文本占位符，不增加/删除节点，不引入外部脚本/样式）”。（未接入）
  - [ ] 回退：若返回非法 HTML，则要求 AI 仅返回 JSON 文本字段（如 `{"reportTitle": "...", "summary": "..."}`），前端再合并进模板。（未接入）
- 提示词要点：
  - [ ] 输入：时间范围、Top 标签（名称/数量/占比）、趋势数据、显著变化阈值等结构化数据 + HTML 模板。（未成稿）
  - 约束：
    - 只替换占位符，不修改模板结构，不插入 `script/link/iframe`。
    - UTF-8、简体中文、语气客观克制；摘要 ≤ 200 字；洞察 3–5 条；避免敏感词与夸张表述。
  - [ ] 产出：优先完整 HTML；失败回退 JSON 字段集合。（未接入）
- 示例系统提示（简化版）：
```
...
## 设置项（建议）
 - [ ] AI：启用开关、API Key、端点 URL、模型名、超时与重试；“仅本地模式”开关。（设置页存在但未接入报告生成流程）
 ...
 - [ ] 隐私：是否允许向 AI 发送聚合数据（默认关闭）。

## 开发任务拆分（对应 TODO）
- [x] 标签采集器（content）与去重
- [x] 聚合与 IndexedDB 封装（`views`/`reports`）
- [ ] 报告生成器（本地统计 + 模板 + AI 填充/回退）。（本地统计 + 模板已完成；AI 填充/回退未完成）
